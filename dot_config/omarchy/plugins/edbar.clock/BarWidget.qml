...
BarWidget {
  ...
  function formatted(date) {
    // Qt.formatDateTime uses Qt's single unified system QLocale, which on
    // this machine resolves from LANG (kept en_US for the English UI) — it
    // has no POSIX-style separate LC_TIME slot, so day/month names always
    // came out English regardless of LC_TIME=pt_BR.UTF-8. date.toLocaleString
    // with an explicit Locale forces just this widget's names to pt_BR.
    return date.toLocaleString(Qt.locale("pt_BR"), activeFormat.replace(/ww/g, Model.isoWeekLiteral(date.getFullYear(), date.getMonth(), date.getDate())))
  }
