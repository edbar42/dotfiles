...
QtObject {
  ...
  readonly property string configPath: Quickshell.env("XDG_RUNTIME_DIR")
    + "/ilyazar-btop.conf"
  // No real Omarchy install here, so there's no packaged theme-synced
  // config to seed from. Use the user's own btop.conf instead, so the
  // private config this plugin writes keeps their real color_theme.
  readonly property string omarchyConfigPath:
    Quickshell.env("HOME") + "/.config/btop/btop.conf"
